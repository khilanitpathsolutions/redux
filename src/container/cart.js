        ...
        <>
          <Row xs={1} sm={2} md={3} lg={4} className="g-4">
            {cartItems[loggedInUsername]?.map((item) => (
              <Col key={item.id}>
                <Card style={{ border: "1px solid black",borderRadius: '20px', marginBottom: "10px" }}>
                  <Card.Body className="d-flex flex-column">
                    ...
                    <div className="d-flex align-items-center">
                      <Button variant="primary" onClick={() => dispatch(decrementQuantity({ username: loggedInUsername, itemId: item.id }))}>
                        -
                      </Button>
                      <span style={{ padding: "6px" }}>{item.quantity}</span>
                      <Button variant="primary" onClick={() => dispatch(incrementQuantity({ username: loggedInUsername, itemId: item.id }))}>
                        +
                      </Button>
                      <div style={{ width: "100%", display: "flex", justifyContent: "end" }}>
                        <Button variant="danger" onClick={() => dispatch(removeItem({ username: loggedInUsername, itemId: item.id }))}>
                          Remove
                        </Button>
                      </div>
                    </div>
          ...
          <div className="mt-4" style={{ background: "#e6fff2", display: "flex", flexDirection: "column", alignItems: "center" }}>
            <h3>Cart-Total</h3>
